...
- **👀 实时预览**：即时查看名片效果
- **💾 双重导出引擎**：
  - **原生Canvas绘制**：完全控制，像素级精确，标准化输出
    - 零变形导出：彻底解决头像挤压和文字位置偏移
    - 高性能渲染：比html2canvas快3-5倍，内存占用更少
    - 多分辨率支持：PNG(3x)、JPG(2x)不同质量级别
  - **DOM-to-image保真**：保持样式，兼容性好，适合复杂布局  
    - 完美样式还原：CSS效果、字体、阴影完全保持
    - 跨域图片处理：自动处理CORS和外部资源
    - 高兼容性：支持各种浏览器和复杂CSS
  - **对比导出模式**：同时使用两种方法，性能测试，选择最佳方案
  - **多格式支持**：PNG高质量、JPG小文件，自动优化
  - **实时导出**：在主界面直接导出，无需跳转页面
